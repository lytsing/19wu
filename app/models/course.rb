class Course < ActiveRecord::Base
  ...
  has_many :participants, :class_name => "CourseOrderParticipant"
  has_many :updates,      :class_name => "CourseChange"
  has_many :tickets,      :class_name => "CourseTicket"
  has_many :orders,       :class_name => "CourseOrder"
  ...
  def self.remind_participants
    Course.upcoming.find_each do |e|
  ...
  def end_time_must_after_start_time
    if end_time.present? && start_time.present? && end_time < start_time
      message = I18n.t('errors.messages.greater_than_or_equal_to', :count => Course.human_attribute_name(:start_time))
