...
describe OrderMailer do
  ...
  let(:order) { create(:order_with_items, require_invoice: true, course: course) }
  let(:trade_no) { '2013080841700373' }
  let(:participant) { order.create_participant }
  let(:refund) { create :course_order_refund, :submited, order: order }
  ...
  describe "notify" do
    describe "user" do
      describe "order created" do
        ...
        context 'free' do
          let(:order) { create(:order_with_items, tickets_price: 0, require_invoice: true, course: course) }
      ...
      describe "order checkin code" do
        ...
        its(:from) { should eql [Settings.email.from] }
        its(:to) { should eql [order.user.email] }
        its('body.decoded') { should match "#{course.title} 签到码：#{participant.checkin_code}。" }
      end
      describe "order refunded" do
        before { order.pay(trade_no) }
        subject { OrderMailer.notify_user_refunded(refund) }
        its(:subject) { should eql "#{course.title} 订单 #{order.number}，已退款 #{refund.amount} 元" }
        its(:from) { should eql [Settings.email.from] }
        its(:to) { should eql [order.user.email] }
    ...
    describe "organizer" do
      describe "order created" do
        ...
        its(:subject) { should eql "#{course.title} 订单 #{order.number}，#{order.user.login} 下单" }
        its(:from) { should eql [Settings.email.from] }
      ...
      describe "order paid" do
        ...
        its(:subject) { should eql "#{course.title} 订单 #{order.number}，#{order.user.login} 完成支付" }
        its(:from) { should eql [Settings.email.from] }
        its(:to) { should eql [course.user.email] }
      ...
      describe "order refunded" do
        ...
        its(:subject) { should eql "#{course.title} 订单 #{order.number}，已向 #{order.user.login} 退款 #{refund.amount} 元" }
        its(:from) { should eql [Settings.email.from] }
        its(:to) { should eql [course.user.email] }
    ...
    describe "support" do
      describe "refund submit" do
        ...
        its(:subject) { should eql "#{course.title} 订单 #{order.number}，主办方申请退款" }
        its(:from) { should eql [Settings.email.from] }
