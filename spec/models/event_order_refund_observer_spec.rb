...
describe CourseOrderRefundObserver do
  ...
  context 'submit' do
    let(:refund) { order.refunds.submit attributes_for(:course_order_refund) }
  ...
  context 'refund' do
    ...
    describe 'order' do
      ...
      context 'paid_amount is zero' do
        let(:refund) { create :course_order_refund, :submited, order: order, amount: order.price }
